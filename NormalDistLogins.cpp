...
void parse() {
  ...
  unsigned int numtests =
      vsize; // this can change to some scalar multiple of vsize if desired

  auto seed = std::chrono::system_clock::now().time_since_epoch().count();
  std::mt19937 random_gen(seed);
  std::normal_distribution<double> normdist(
      vsize / 2,
      vsize / 5); // mean needs to be vsize/2. stdev can change if needed

  for (unsigned int i = 0; i < numtests; i++) {
    double unref_index = normdist(random_gen);
    unsigned int ref_index;
    while (unref_index < 0 ||
           unref_index > vsize) {         // while value is not in proper range
    ...
    outdata << creds[ref_index].first << '\t' << creds[ref_index].second
            << endl;
  }
