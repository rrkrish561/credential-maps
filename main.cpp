...
int main() {
  ...
  while (menuSelect != 4) {
      ...
      if (treeSpecifier == 1) {
        ...
        if (treeSpecifier == 2) {
          ...
          if (splayInserter.verifyCreds(userName, passWord)) {
            auto t2 = Clock::now();
            cout << "Account found in Splay Tree in "
                 << chrono::duration_cast<chrono::nanoseconds>(t2 - t1).count()
                 << " nanoseconds." << endl;
          } else {
    ...
    case 3: {
      ...
      cout << "Total time to generate BST: " << time << " milliseconds."
           << endl;
      ...
      cout << "Total time to generate Splay Tree: " << time << " milliseconds."
           << endl;
      ...
      // search for simulated logins in BST and check average time for one user
      // to log in
      t1 = Clock::now();
      ...
      cout << "Average login time in BST is " << averageTime << " microseconds."
           << endl;

      // search for simulated logins in SplayTree and check average time for one
      // user to log in
      t1 = Clock::now();
      for (unsigned int i = 0; i < logins.size(); i++) {
      ...
      cout << "Average login time in Splay Tree is " << averageTime
           << " microseconds." << endl;
